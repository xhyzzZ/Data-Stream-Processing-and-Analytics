...
public abstract class JobEventTestBase<OUT> {
	...
	protected List<OUT> runApp(TestJobEventSource rides, TestSink<OUT> sink, Testable solution) throws Exception {
		AppBase.jobEvents = rides;

		return execute(sink, solution);
	}

	protected List<OUT> runApp(TestJobEventSource rides, TestStringSource strings, TestSink<OUT> sink, Testable exercise, Testable solution) throws Exception {
		AppBase.jobEvents = rides;
